...
namespace GIGLS.Services.Implementation.Shipments
{
    public class PreShipmentMobileService : IPreShipmentMobileService
    {
        ...
        public async Task<PreShipmentMobileDTO> AddMobilePickupRequestNew(MobilePickUpRequestsDTO pickuprequest)
        {
            try
            {
                var newPreShipment = new PreShipmentMobileDTO();
                ...
                //status not send if the shipment is accepted
                if(pickuprequest.Status == null || pickuprequest.Status == "")
                {
                    pickuprequest.Status = MobilePickUpRequestStatus.Accepted.ToString();
                }
                ...
                {
                    string shipmentstatus = preshipmentmobile.shipmentstatus.ToLower();

                    switch (shipmentstatus)
                    {
                        case "shipment created":
                            newPreShipment = await MobilePickupRequestForShipmentCreation(preshipmentmobile, pickuprequest);   
                            break;
                        case "accepted":
                            AddMobilePickupRequest(pickuprequest);  
                            break;
                        case "proceedtopickup":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Arrived", "Cancel"]
                            break;
                        case "arrived":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Visited", "Rejected", "Dispute", "Confirmed"]
                            break;
                        case "confirmed":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Enroute Delivery"]
                            break;
                        case "enroute delivery":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Delivered", "LogVisit"]
                            break;
                        case "delivered":
                            AddMobilePickupRequest(pickuprequest); // Allowed Status updates ===> [] NO Update
                            break;
                        case "dispute":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Resolved", "Cancelled"]
                            break;
                        case "resolved":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["Enroute Delivery", "Dispute"]
                            break;
                        case "cancelled":                                           
                            AddMobilePickupRequest(pickuprequest); //Partner Cancel: CancelShipmentNoCharge --> Allowed Status updates ===> ["TimeOut", "Missed", "Accepted", "Rejected"]
                            break;
                        case "rejected":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> ["TimeOut", "Missed", "Accepted", "Rejected"]
                            break;
                        case "visited":
                            AddMobilePickupRequest(pickuprequest); //Allowed Status updates ===> []
                            break;
                        default:
                            break;
                    }
        ...
        //Allowed Status updates ===> ["TimeOut", "Missed", "Accepted", "Rejected"]
        private async Task<PreShipmentMobileDTO> MobilePickupRequestForShipmentCreation(PreShipmentMobile preshipmentmobile, MobilePickUpRequestsDTO pickuprequest)
        {
            if (pickuprequest.Status == MobilePickUpRequestStatus.Rejected.ToString() || pickuprequest.Status == MobilePickUpRequestStatus.TimedOut.ToString()
                || pickuprequest.Status == MobilePickUpRequestStatus.Missed.ToString() || pickuprequest.Status == MobilePickUpRequestStatus.Accepted.ToString())
            {
                await _mobilepickuprequestservice.AddOrUpdateMobilePickUpRequests(pickuprequest);
            ...
            {
                throw new GenericException($"Shipment not allowed t0 be {pickuprequest.Status}");
            }           
        }

        ////Allowed Status updates ===> ["Proceed to Pickup", "Cancel"]
        private async Task<PreShipmentMobileDTO> MobilePickupRequestForShipmentAccepted(PreShipmentMobile preshipmentmobile, MobilePickUpRequestsDTO pickuprequest)
        {
            if (pickuprequest.Status == MobilePickUpRequestStatus.ProceedToPickUp.ToString() || pickuprequest.Status == MobilePickUpRequestStatus.Cancelled.ToString())
            {
                await _mobilepickuprequestservice.AddOrUpdateMobilePickUpRequests(pickuprequest);

                if(pickuprequest.Status == MobilePickUpRequestStatus.ProceedToPickUp.ToString())
                {
                    //Update Activity Status
                    await UpdateActivityStatus(pickuprequest.UserId, ActivityStatus.OnDelivery);
                }
